...
const GuestDashboard = () => {
  ...
  useEffect(() => {
    const fetchApprovedWeddings = async () => {
      setLoading(true);
      try {
        const response = await fetch(`https://savethedate-gzy9.onrender/api/guests/${userId}/approved-weddings`);
  ...
  const handlePayForWedding = async (weddingId) => {
    ...
    try {
      const response = await fetch(`https://savethedate-gzy9.onrender/api/payments/create-checkout-session`, {
        method: 'POST',
        headers: {
